...
Control.Window = Class.create({
    ...
    open: function(event){
        ...
        if(this.href && !this.options.iframe && !this.remoteContentLoaded){
            ...
            if(this.options.imageHandle==true || this.href.match(/\.(jpe?g|gif|png|tiff?)$/i)){
            ...
            if(this.options.indicator)
                this.showIndicator();
			this.notify('onInsertFrame'); // added by Mehran Ahadi (need for ArtaTagsHtml::ModalWindow)
            // some codes added by MEHRAN AHADI to enable window.close() function in iframes to close livepipe windows. 
			Control.Window.iframeTemplate = new Template('<iframe src="#{href}" name="#{name}" width="100%" height="100%" frameborder="0"></iframe>');  // added by MEHRAN AHADI   
            var frameName='window_iframe_'+Math.random(); // added by MEHRAN AHADI
            this.getRemoteContentInsertionTarget().insert(Control.Window.iframeTemplate.evaluate({
                href: this.href,
                name: frameName // added by MEHRAN AHADI
            }));
			var frameObj=window.frames[frameName];// added by MEHRAN AHADI
            //var iframe = this.container.down('iframe');
            //iframe.onload = function(){
			// *** removed by MEHRAN AHADI because it fails on IE8 ***
           	frameObj.onload = function(){
                this.notify('onRemoteContentLoaded');
                if(this.options.indicator)
                    this.hideIndicator();
                frameObj.onload = null; // edited by MEHRAN AHADI: was "iframe" instead of "frameObj"
            	frameObj.close=this.closeHandler; // added by MEHRAN AHADI
            }.bind(this);
			// end edited area by MEHRAN AHADI
        }
    ...
    position: function(event){
        ...
        if(this.options.position == 'mouse'){
            ...
            /* ADDED BY MEHRAN AHADI AS A TWEAK */
								...
								if($value(this.options.offsetTop)==0){
									this.options.offsetTop=15;
								}
								
			/* end ADDED BY MEHRAN AHADI AS A TWEAK */
			...
			/* ADDED BY MEHRAN AHADI AS A TWEAK */
								vo=this.container.viewportOffset();
								
								vd=document.viewport.getDimensions();
								
								
								hei=this.container.getHeight();
								xx=vo.top+ hei - vd.height;
								if(xx > 0){
									t_toset= parseInt(this.container.getStyle('top')) - xx;
									this.container.setStyle({
										top : t_toset + 'px'
									});
								}
								...
								if(xx>0){
									wid=this.container.getWidth();
									if(vo.left+ wid - vd.width > 0){
										l_toset= vo.left - wid - (2*this.options.offsetLeft);
										this.container.setStyle({
											left : l_toset + 'px'
										});
									}
									...
									wid=this.container.getWidth();
									xx=vo.left+ wid - vd.width;
									if(xx > 0){
										l_toset= parseInt(this.container.getStyle('left')) - xx;
										this.container.setStyle({
											left : l_toset + 'px'
										});
									}
