...
define('STEP_DB', 'مشخصات پایگاه داده');
...
define('STEP_DB_I', 'اتصال آرتا به پایگاه داده');
...
<p>به فرآیند نصب آرتا خوش آمدید. شما به کمک این فرآیند کوتاه و آسان، می توانید یک کپی از سیستم<b> آرتا نسخه %s</b> را روی میزبان خود نصب نمایید.</p>
<p>لطفاً توجه داشته باشید که در حین نصب، از دکمه ی Back مرورگر خود<b> استفاده ننمایید</b>.</p>
HTML
);
...
<p>آرتا به یک پایگاه داده MySQL بعنوان ذخیره گاه اطلاعات احتیاج دارد. استفاده از سرور MySQL نسخه 4.1.2 یا بالاتر ضروری است، اما MySQL 5 یا جدید تر توصیه می شود. برای اتصال موفقیت آمیز به سرور، آرتا نیازمند نام میزبان (Host Name) سرور، نام کاربری و رمز عبور پایگاه داده و نام آن است. شما می توانید این اطلاعات را با مراجعه به وبسایت شرکت ارائه دهنده ی میزبان و یا پرسش از اپراتورهای آن فراهم کنید.</p>
HTML
);
...
define('DB_HOST', 'میزبان پایگاه داده (DB Host)');

define('SET_DB_NAME', <<<HTML
<p>نام پایگاه داده را در اینجا وارد کنید. اگر هنوز پایگاه داده نساخته اید، با نامی مشخص، یک پایگاه داده ایجاد کنید. توصیه می شود که پایگاه داده مورد استفاده خالی از هر گونه جدول باشد. فراموش نکنید که Database Collation را برابر <code>utf8_general_ci</code> قرار دهید.</p>
HTML
);
define('DB_NAME', 'نام پایگاه داده (DB Name)');

define('SET_DB_CREDENTIALS', <<<HTML
<p>برای اتصال به سرور، به یک حساب کاربری نیاز دارید که علاوه بر دسترسی به پایگاه داده ساخته شده، توانایی اجرای دستورات <dfn title="SELECT,INSERT,UPDATE,DELETE">مدیریت اطلاعات</dfn> و <dfn title="CREATE,ALTER,INDEX,DROP">ساختار</dfn> را داشته باشد.</p>
HTML
);
...
<p>این همان پیشوند نام جداول داخل پایگاه داده است که توصیه می شود آن را تغییری ندهید. برای مثال اگر <code>arta_</code> را انتخاب کنید، جدول "Users" با نام  <code>arta_users</code> ساخته خواهد شد.</p>
HTML
);
...
define('INVALID_DB_INFO', 'خطا: اتصال به پایگاه داده موفقیت آمیز نبود. <br/>پیام سرور: "%s"');
...
define('CANNOT_EXECUTE_TEST_QUERIES', 'دستورات مدیریت اطلاعات با موفقیت اجرا نشدند. مطمئن شوید که کاربر پایگاه داده توانایی اجرای دستورات ذکر شده را داشته باشد، همچنین، Collation پایگاه داده برابر با utf8_unicode_ci باشد.');
