<?php
/**
 * This file contains an standalone unzipping class based on dUnzip2 class by Alexandre Tedeschi.
 * 
 * @author		Mehran Ahadi
 * @package		Arta
 * @subpackage	ArtaArchive
...
##############################################################
/**
 * Unzipper for ArtaArchive based on dUnzip2 class.
 * This is a modified version of Alexandre Tedeschi's dUnzip2 class.
 */
class ArtaArchive_unzip{
	Function getVersion(){
		return "2.662";
	}
	// Public
	...
	// Public
	Function ArtaArchive_unzip($fileName){
	...
	Function unzip($compressedFileName, $targetFileName=false, $applyChmod=0777){
		$compressedFileName=(string)$compressedFileName; // added
		if(!sizeof($this->compressedList)){
		...
		if($applyChmod && $targetFileName)
			chmod($targetFileName, $applyChmod); //A bug fixed (was chmod($targetFileName, 0777);).
			...
			if(!is_dir($outDN) && $maintainStructure){
				...
				foreach($folders as $folder){
					...
					if(!is_dir("$targetDir/$str")){
						...
						if(mkdir("$targetDir/$str")==false){return false;}
						if($applyChmod)
