...
/**
 * This class is base class which "sockets" or "pure" classes should be used while extending this class.
 */
class ArtaFTP_base {
	/* Public variables */
	var $LocalEcho;
	...
	function __construct($port_mode=FALSE, $verb=FALSE, $le=FALSE) {
		...
		$this->AutoAsciiExt=array("1ST","ASP","BASH*","BAT","C","CFM","CGI","CPP","CSS","CSV","DHTML","FILE","H","HTACCESS","HTM","HTML","HTPASSWD","INI","JS","MAK","MSG","OLD","PHP","PINERC","PM","QMAIL","QMAIL-*","README","SCREENRC","SETUP","SH*","STYLE","THREADS","TMPL","UBB","INC","XML","SHTML","LOG","PHP3","PHTML","PL","PERL","SH","SQL","TXT"); // extended
...
//	Parses 1 line like:		"drwxrwx---  2 owner group 4096 Apr 23 14:57 text"
	...
	function restore($from) {
		...
		if(!$this->_exec("REST ".$from, "restore")) return FALSE; // was resore
		if(!$this->_checkCode()) return FALSE;
		return TRUE;
	}
	...
	function nlist($pathname="", $arg="") { // ', $arg=""' was missing
...
$mod_sockets=TRUE;
if (!extension_loaded('sockets') && function_exists('dl')) {
	$prefix = (PHP_SHLIB_SUFFIX == 'dll') ? 'php_' : '';
	if(!@dl($prefix . 'sockets.' . PHP_SHLIB_SUFFIX)) $mod_sockets=FALSE;
}elseif(!extension_loaded('sockets')){
	$mod_sockets=FALSE;
}
require_once "ftp_".($mod_sockets?"sockets":"pure").".php";
?>